...
- **Play demo GIF…** opens the included demo animations:
  - **Original dot + gray gradient** preserves the first playback test for comparison.
  - **Moving gray clouds** is a slow, Perlin-like multi-scale grayscale field.
  - **Moving gray wave** uses broad overlapping gray waves without a clipped object.
  - **Soft circle with clockwise gradient** has a large feathered circle and a
    smaller feathered circular gradient orbiting clockwise inside it.
  - **Soft gray ripples** sends gentle grayscale ripples through the patch.
...
The bundled demo GIFs are 128×96, 40 frames, 50 ms per frame, and use a fixed
64-level grayscale palette. They are generated by `tools/build_demo_gifs.py` so
the source animations remain reproducible and easy to adjust.
